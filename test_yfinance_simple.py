...
async def test_yfinance_simple():
    ...
    try:
        ...
        # 2. 测试实时数据
        print("\n📈 测试实时数据")
        real_time = await data_manager.get_real_time_data(
            symbol='US.AAPL',
            data_source='yfinance'
        )
        if real_time:
            print(f"✅ 实时数据获取成功")
            print(f"   最新价格: {real_time['last_price']:.2f}")
        else:
            print("ℹ️  yfinance不支持实时数据（这是正常情况）")
        
        # 3. 测试批量数据
        print("\n📦 测试批量数据获取")
        symbols = ['US.AAPL', 'US.MSFT', 'HK.00700']
        ...
        print(f"✅ 批量获取成功: {len(batch_data)}/{len(symbols)} 个符号")
        print(batch_data)
        # 4. 测试数据源信息
        print("\nℹ️  测试数据源信息")
